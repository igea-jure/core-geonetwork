...
<xsl:stylesheet version="2.0"
    ...
    <xsl:for-each select="gmd:identificationInfo/*">
      ...
      <xsl:for-each select="gmd:citation/gmd:CI_Citation">
        ...
        <xsl:for-each select="gmd:identifier/gmd:RS_Identifier/gmd:codeSpace/gco:CharacterString">
          <Field name="identifierCS" string="{string(.)}" store="true" index="true"/>
        </xsl:for-each>
  ...
  gmd:pointOfContact/gmd:CI_ResponsibleParty/gmd:individualLastName/gco:CharacterString">
      ...
      <xsl:for-each select="gmd:resourceMaintenance/
        ...
        <xsl:variable name="updateFrequencyCodeList" select="string(.)"/>
        <xsl:variable name="updateFrequencyTranslation"
                      select="util:getCodelistTranslation('gmd:MD_MaintenanceFrequencyCode', $updateFrequencyCodeList, string($isoLangId))"/>

        <Field name="updateFrequency" string="{$updateFrequencyTranslation}" store="true" index="true"/>
      </xsl:for-each>

      <xsl:for-each select="gmd:resourceMaintenance/
                            gmd:MD_MaintenanceInformation/gmd:maintenanceNote//gmd:LocalisedCharacterString[@locale=$langId]">
        <Field name="updateNote" string="{string(.)}" store="true" index="true"/>
      </xsl:for-each>
      ...
      <xsl:for-each select="gmd:resourceConstraints/*">
        ...
        <xsl:for-each select="current()[gmd:accessConstraints]/gmd:otherConstraints/gmx:Anchor">
          <Field name="{$fieldPrefix}AccessConstraints"
                 string="{util:getCodelistTranslation('gmd:accessConstraints', string(./@*), string($isoLangId))}" store="true" index="true"/>
        </xsl:for-each>

        <xsl:for-each select="gmd:otherConstraints//gmd:LocalisedCharacterString[@locale=$langId]">
          <Field name="{$fieldPrefix}OtherConstraints"
                 string="{string(.)}" store="true" index="true"/>
        </xsl:for-each>

        <xsl:for-each select="current()[gmd:useConstraints]/gmd:otherConstraints/gmx:Anchor">
          <Field name="{$fieldPrefix}UseConstraints"
                 string="{util:getCodelistTranslation('gmd:otherConstraints', string(./@*), string($isoLangId))}" store="true" index="true"/>
        </xsl:for-each>

        <xsl:for-each select="current()[gmd:useConstraints]/gmd:otherConstraints//gmd:LocalisedCharacterString[@locale=$langId]">
          <Field name="{$fieldPrefix}UseConstraints"
                 string="{string(.)}" store="true" index="true"/>
        </xsl:for-each>

        <xsl:for-each select="gmd:useLimitation//gmd:LocalisedCharacterString[@locale=$langId]">
    ...
    <xsl:for-each select="gmd:dataQualityInfo/*/gmd:lineage/*/gmd:statement//gmd:LocalisedCharacterString[@locale=$langId]">
      <Field name="lineage" string="{string(.)}" store="true" index="true"/>
    </xsl:for-each>

    <xsl:for-each select="gmd:dataQualityInfo/*/gmd:report/*/gmd:result/gmd:DQ_ConformanceResult/gmd:specification//gmd:LocalisedCharacterString[@locale=$langId]">
      <xsl:if test="$inspire='true'">
        <!--
      ...
      <xsl:variable name="conformityTitle" select="../../../../../../../gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:title//gmd:LocalisedCharacterString[@locale=$langId]"/>
      ...
      <xsl:variable name="conformityExplanation" select="../../../../../../../gmd:DQ_ConformanceResult/gmd:explanation//gmd:LocalisedCharacterString[@locale=$langId]"/>
      <xsl:variable name="conformityPass" select="../../../../../../../gmd:DQ_ConformanceResult/gmd:pass/gco:Boolean/text()"/>
      <xsl:variable name="conformityCode" select="../../../../../../../gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:identifier/gmd:RS_Identifier/gmd:code/gco:CharacterString/text()"/>
      <xsl:variable name="conformityCodeSpace" select="../../../../../../../gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:identifier/gmd:RS_Identifier/gmd:codeSpace/gco:CharacterString/text()"/>
      <Field name="conformity" string="{concat($conformityTitle, '|', $conformityDate, '|', $conformityExplanation, '|', $conformityPass, '|', $conformityCode, '|', $conformityCodeSpace)}" store="true" index="true"/>
    </xsl:for-each>
